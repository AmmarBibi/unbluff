...
The theme of this release is **the checkers were wronger than the code**. Most of what follows
was found by an instrument catching another instrument, and several were found by mutating a
test to see whether it could fail at all. Working notes, at maintainer depth, are in
[`docs/audits/changelog_v1_4_0_engineering_log.md`](docs/audits/changelog_v1_4_0_engineering_log.md).

### Security

- **`SECURITY.md`**, stating the execution model plainly and routing reports through GitHub
  private advisories. unbluff runs your project's test command - that is the point of it - so
  the question was never whether it executes code but *whose, and on whose say-so*.
- **The turn-end hook now discloses what an auto-detected command will actually run**, once per
  project, before the first run. Not the wrapper: `npm test --silent` and `python -m pytest`
  name nothing. It prints the `scripts.test` **body** and the **`conftest.py` files pytest will
  import** - which is the part that can be hostile, since pytest imports conftest before
  collecting anything. Keyed on the disclosed content, so a repository that changes what runs
  is disclosed again rather than staying silent on a licence it earned once.
- **The push gate no longer auto-detects in repositories that never opted in.**
  `--install-global` fires in every repository on the disk, including ones cloned only to read,
  where nothing implies consent. Those must name a command in `.claude/pre-push.cmd`. A
  repository where you ran `--install` keeps auto-detect - that install *is* the consent. The
  refusal says so and **allows** the push; it never blocks on "you did not configure me".

### Fixed

- **A moved or deleted clone used to block every `git push` on the machine.** Both shims embed
  an absolute path; when it went stale the shim ran a missing script, returned nonzero, and
  blocked pushes in every repository, with no usable diagnosis, and `install.py --uninstall`
  could not help because it never set `core.hooksPath`. The shims now fail **loud but open** -
  `NOTHING WAS VERIFIED` on every push, and the push is allowed. `--install-global --remove`
  also deletes the dispatchers it wrote.
- **`consistency-audit` certified scanned PDFs CLEAN without reading a character.** The PyMuPDF
  branch returned its result unconditionally while the other two readers checked it was
  non-empty, so an image-only PDF yielded `"\n\n\n"`, no numbers were found, and the document
  came back clean. A scan is now refused with an OCR instruction. This is the twin of the Word
  text-box defect fixed in 1.3.1, in the same file, and it shares its shape: **installing the
  better library made the audit read less**.
- **`hook-provenance` ran in `--selftest` mode only**, so the half that actually reads git's
  wiring on your machine was invoked by nothing - not the suite, not CI, not the push path.
- **The piped-gate guard did not exist for PowerShell users.** It was registered `matcher:
  "Bash"` and blind to `Select-Object -First`, which tears a gate down before it finishes.
- **`timing_claim_guard` ignored `UNBLUFF_STATE_DIR`**, the only stateful hook that did.
- **`duplicate_registration_check` read any ALL-CAPS tuple of strings as a dispatcher roster.**

### Added

- **The README's own numbers are now gated.** It claimed "eighteen pieces" while listing 17 and
  shipping 20 - and two of the missing three (`piped_gate_guard`, `timing_claim_guard`) are
  hooks that fire on your machine while being undocumented. `check_readme_fresh` now derives
  the piece **count and roster** from the installer's wiring and fails on either. Third
...
- **The mutation harness can pin a fix that only SAYS something**, by asserting a marker in the
  ...
  invocation. Six behaviours had sat unpinned because every entry ran `--selftest`.
- **A gate's registered mode is derived and checked**, after two gates were found registered in
  a mode that could not fail.
- **False-alarm rate is measured, not asserted**, across all 11 wired hooks with a firing corpus.

### Changed

- `tools/mutation_check.py` split 1415 -> 377 lines; the entries table moved to two siblings.
- `[Unreleased]`'s 19 KB of working notes moved to `docs/audits/`, linked above.

### Known and not silently omitted

- `hook-provenance` fails inside a git **worktree** by construction: it compares the wired path
  against the containing repository, and a worktree's path is not the wired one. Byte-identical
  code, zero AST delta. Green in a normal checkout, which is what CI runs.
- The `install.py` selftest (299 of its 927 lines) and `pre_push_gate_selftest.py` (1192 lines)
  both want splitting. The size ratchet is starting to steer where new code goes, which is
