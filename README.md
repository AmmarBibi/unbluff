...
*The demos below are animated reconstructions generated by [`scripts/make_demos.py`](scripts/make_demos.py). Hook output is shown verbatim; the `rate_prompt` and `meta-review` demos illustrate a representative result (the model's rating and the skill's report naturally vary).*
