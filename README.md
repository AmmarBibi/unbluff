...
Catches a success claim ("it works", "tests pass") made with zero tool runs, and nudges the agent to actually verify or soften it.
...
Rates every prompt X/10 and rewrites it to a sharper version inline before the agent acts - with no extra model call. It spends a few tokens on the inline rating, so it has an off-switch (`CLAUDE_RATE_PROMPTS=off`).

![rate_prompt scoring a prompt and rewriting it inline](docs/rate-prompt.gif)

### meta-review · skill
A deliberate reasoning pass that audits for parked work, instance-only fixes, optimization gaps, and what's silently missing.

![meta-review example report](docs/meta-review.png)

### fast_test_on_stop · Stop
When source changed, runs your fast tests at the end of a turn and feeds any failure back to the agent.

![fast_test_on_stop running the tests and catching a regression](docs/fast-test.gif)

### meta_audit_on_stop · Stop
Surfaces parked / deferred / TODO work that has no decision, plus unpushed commits.

![meta_audit_on_stop output](docs/meta-audit.png)

### memory_hygiene_guard · Stop
Flags rot in Claude Code's auto-memory (index bloat, stale commit hashes, evolving state).

![memory_hygiene_guard output](docs/memory-hygiene.png)

### hook_health_check · SessionStart
At session start, verifies your configured hooks resolve, and weekly-runs each hook's self-test. (It never judges or modifies your other hooks.)

![hook_health_check output](docs/hook-health.png)

### stop_dispatcher · Stop
Runs the turn-end hooks in a single process and logs a fire-ledger of what fired.
