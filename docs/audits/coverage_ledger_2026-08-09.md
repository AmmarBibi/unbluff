...
| **`FA-MEMHYG`** - the first false alarm criterion 3 has ever caught | **SCHEDULED** (MEDIUM) | `memory_hygiene_guard` fires on a turn ending in a CLEAN project: *"could not check this project's memory: no memory dir for this project"*. Every turn end, for any user without a memory dir. **Deliberately not patched in the same change**, and the reason is on record: it is genuinely ambiguous rather than obviously wrong - this repo's own rule is that a check which CANNOT RUN must say so rather than pass vacuously. `fast_test_on_stop` already resolved the identical tension with `_nogate_reason()` once-per-`(path, reason)` notices, and that pattern is the fix |
