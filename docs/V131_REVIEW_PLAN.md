...
#### HB-1 and A2, 2026-08-06 - found by running the integration suite ON WINDOWS

The integration CI job runs on **ubuntu-latest only**. Both findings below were invisible there
and visible on the first local Windows run. **This is P13 F's shape exactly** - "an ubuntu-only
mutation job proves them NOWHERE while printing a clean summary" - which this repo already
diagnosed and fixed for the MUTATION jobs by adding a Windows mirror, and never applied to the
integration job. The lesson was on the shelf; it got re-learned.

| id | sev | owner | item |
|---|---|---|---|
| **HB-1** | **HIGH - SHIP-BLOCKING** | **FIXED 2026-08-06** | **`hook_health_check` reports a WORKING hook command as a missing executable, on the one line the user reads at every SessionStart.** `check_config` resolves a command's first token with `shutil.which()`, which searches PATH for a FILE. Claude Code runs hook commands through a SHELL, so a shell builtin is a perfectly valid hook - and `echo` is both the canonical trivial hook and a cmd.exe builtin with no `echo.exe`. MEASURED before fixing: `check_config({"Stop": [... "echo other"]})` returned `['Stop: executable not on PATH: echo']` on win32 while `echo other` ran with rc=0 through the shell; `shutil.which` returns None for `echo`, `cls`, `set`, `dir`, `type`, `cd`, `copy`, `call` and every other cmd builtin. **Ship-blocking by the re-scoped gate on both halves: R1** - `hook_health_check` is a wired SessionStart hook that fires every session - **and R2** - the trigger is the USER's own `settings.json`, a hook they wired themselves. The consequence is a permanent false problem on a CORRECT config, which is **B3-FP's recorded rule** ("a guard which false-alarms on a correct config gets disabled, which is strictly worse than none") happening to the suite's most visible line. FIX: the builtin sets are consulted before reporting, as **FROZEN EXTERNAL VOCABULARIES** rather than rosters - cmd.exe's and POSIX sh's builtin sets are fixed by their specifications, so they do not rot the way a roster over this repo's own code does, and a name absent from them still has to resolve on PATH. Regression test written FIRST and watched FAIL on the real message. **Both directions asserted**, because a "fix" that stopped reporting genuinely missing executables would pass the first half while being useless. The POSIX half of the fixture uses `export` rather than `echo` - `/bin/echo` is a real file, so asserting `echo` there would pass through `shutil.which()` and prove nothing, whereas `export` must be builtin to work at all. **That half is proven by CI or nowhere.** Mutations `HB1a` (the exemption is removed) and `HB1b` (the exemption widens to every name) both CAUGHT |
| **A2** | **HIGH - it was RED on main, not latent** | **FIXED 2026-08-06** | **A twin roster, in the file that already documents why twins are forbidden.** `tests/test_integration.py` asserted a hardcoded `== 4` unbluff hook groups; `piped_gate_guard` (`3cea480`) made it five, and the integration job went red on `main` and stayed red. The same file's `_install_skill_names()` sits eleven lines above it with the docstring "A second copy of this list is a twin waiting to drift" - **A2 was simply the copy nobody converted**, which is the identical shape as `REQUIRED_HOOKS` in `install.py` ("the third copy of the roster was simply never converted"). FIX is not `4` -> `5`: the expectation is DERIVED from `install.py:desired_groups()`, and the assertion compares **SETS of ids, not a count** - a count cannot tell a missing group from an unexpected extra one, and the count is exactly what drifted. **NOT mutation-pinned, and that is a finding rather than an omission:** `mutation_check.run()` verifies a mutation by running a unit's `--selftest`, and `tests/test_integration.py` has none, so the harness cannot reach it. **The whole 30-scenario integration suite is therefore covered by ZERO mutations - nothing anywhere proves that any of its scenarios can fail.** Rowed below rather than papered over |

| id | sev | owner | item |
|---|---|---|---|
| **INT-MUT** | **MEDIUM** | **v1.4 backlog, phase 2 with the `mutation_check` remainder** | **The integration suite is mutation-covered by nothing.** 30 scenarios, 0 mutations. A scenario that silently stopped asserting - or one that can no longer fail, as A2 could not until it drifted - would keep printing `30/30 scenarios passed`, which is this repo's definition of the absence of evidence. Blocked on a mechanism: `mutation_check` verifies through a unit's `--selftest`, so either `test_integration.py` grows one (it is a script, not a unit) or the harness learns to verify through a command. Not built this session: the surface is frozen, and this adds gate surface |
| **INT-WIN** | **MEDIUM** | **v1.4 backlog, phase 1 with the roster-shaped checks** | **The integration CI job is ubuntu-only, and it hid two live defects.** `.github/workflows/selftest.yml` runs `integration` on `ubuntu-latest` alone. HB-1 and A2 were both invisible there and both visible on the first Windows run. P13 F recorded this exact reasoning for the mutation jobs and added `mutations-windows` as the mirror; the integration job never got one. The fix is the same one this repo already chose once - mirror the job on `windows-latest` - and it is a CI-config change, not new gate surface. Not done in the same commit as HB-1 deliberately: adding the mirror while the defect it would have caught is still being fixed makes it impossible to see the mirror actually catch anything |

| id | sev | owner | item |
|---|---|---|---|
| **M3-LIVE** | **LOW** | **next commit after HB-1** | **The 800-line rule is now VIOLATED, deliberately and out loud.** M3 recorded `hook_health_check.py` at **790** lines specifically "so the next addition to either is a deliberate decision rather than the edit that quietly crosses the line". HB-1 took it to **858**. That is N3's complaint ("the rule re-breaks silently") avoided by exactly one line of foresight. Remedy is the established precedent, not a new invention: split the selftest into `hook_health_check_selftest.py` as `fast_test_on_stop` and `pre_push_gate` already do, listed in `KNOWN_NO_SELFTEST` so it adds **no new gate** and the suite stays at 32. Deferred by ONE commit on purpose - the split carries `fast_test_on_stop_selftest.py`'s documented REBINDING trap (a `global` in the child rebinds a name the parent's production code never reads, so the test passes while testing nothing - A9's class), and that risk must not ride in the same commit as a ship-blocking fix |

#### W-MA1 FIXED, CI GREEN, and three instrument failures - 2026-08-06

**CI is green on `0cfca9d`, the first green run since 2026-08-05 19:10.** 14 of 14 jobs.
Verified by job list rather than by the run's summary conclusion: 11 matrix jobs, integration,
and BOTH mutation harnesses. **Ubuntu sweep 130 of 132 executed, 0 skipped, 2 not-runnable-here,
0 unproven; Windows sweep identical with the same 2 correctly SKIPPED.** `pre_push_gate #30` and
`fast_test_on_stop #D10c` - the two posix-only mutations - both **CAUGHT on ubuntu**, so every
one of the 132 entries is now proven somewhere and 0 SURVIVED.

| id | sev | owner | item |
|---|---|---|---|
| **W-MA1** | **HIGH - was the only SHIP-BLOCKING item of the 21** | **FIXED 2026-08-06 (`116163d`)** | **`_ALLOW_BRACKET_RE` accepted an allow-word ANYWHERE inside any bracket**, so `- PARKED: port the (closed-loop) controller` in the USER's own plan read as a decision TAG - the line was suppressed AND subtracted from the reported total, so the hook under-counted as well as under-reported. Fires through `stop_dispatcher` at every turn-end. This is **P13 B3's defect surviving in the branch P13 B3 did not cover**: that pass fixed the unbracketed cases and added four fixtures for them, and left the bracket branch with no fixture and no mutation at all. **THE PRESCRIBED FIX DOES NOT FIX THE CASE IT NAMES.** `p14_new_code_review.md` prescribes anchoring the tag to the head of the bracket and proposes `(closed-loop)` as the fixture to arm it - but anchoring alone still matches that string, because `closed` IS at the head. MEASURED on a 12-case matrix before adopting anything: **current 6 of 12, prescribed 7 of 12, adopted 12 of 12.** Applying the prescription with its own suggested fixture would have produced a test that failed on arrival. The missing constraint is what separates a tag from a compound word: the allow-word must be followed by a separator or the bracket's end - `(closed)` is a tag, `(closed-form)` is prose. TDD: nine fixtures added FIRST, five watched to FAIL while the four must-suppress cases already passed, so the fix could not cheat by deleting the branch. Mutations `WMA1a` (revert to matching anywhere) and `WMA1b` (revert to the PRESCRIBED form) pin the two constraints separately, so the measurement is encoded rather than only written down |

**Three instrument failures this session, none in the product.** The count matters: the
2026-08-06 tally was nine, and this is the pattern `tooling-discipline` section 4 exists for.
...
| id | sev | owner | item |
|---|---|---|---|
| **ENC-1** | **MEDIUM** | **v1.4 backlog, phase 1 - DESIGN CRITIQUE FIRST** | **Every hook prints under the process locale encoding, so one character outside the user's console codepage replaces its whole report with a crash warning that names none of it.** Confirmed by the review against `hook_health_check`'s problem list; MEASURED as a CLASS here: **24 of 24 `hooks/*.py` lack any stdout reconfiguration**, and the ONE script in the repo that has it (`skills/consistency-audit/scripts/audit.py:25-34`) documents exactly this consequence - "the mechanical pass had never once completed on a Windows machine (verified 2026-07-29)". Trigger is purely the user's own environment: a cp1252 console plus a CJK/Cyrillic/Greek/emoji character in a repo path, a username, a plan line or a hook script path - or `LANG=C` on POSIX. **NOT FIXED THIS SESSION, and not because it is inconvenient.** It is MEDIUM, so it does not block a HIGH-only gate; and the fix has a real design choice that this repo's own rule says to critique before coding: the naive form duplicates a reconfigure block into 24 files, creating 24 twins of the kind three separate findings in this plan already exist to remove. The right granularity is almost certainly the **8 wired process entry points** (`install.py:desired_groups()`'s scripts, plus `pre_push_gate`), because the two dispatchers run their sub-hooks IN-PROCESS and so cover 7 more between them - which makes this ~8 sites rather than 24, and makes the roster DERIVED from the same entry-point set clause 4 above now defines. Rowed with that analysis so the next session starts from a design rather than from a sed command |

### Source-coverage audit, 2026-08-08 - the plan's own arithmetic did not close

**Source:** `docs/audits/adversarial_review_2026-08-06_guards.md`. **Denominator: 42 confirmed
findings, counted directly from the source - 0 CRITICAL, 10 HIGH, 26 MEDIUM, 6 LOW.**

| id | sev | owner | item |
|---|---|---|---|
| **AR6-COUNT** | **MEDIUM** | **FIXED 2026-08-08** | **Three confirmed findings were counted NOWHERE, and the plan contradicted itself about it.** Row `AR-6` carried "MEDIUM x23, LOW x6" and "the remaining 29 confirmed findings", while the same section's corrected total says **42 confirmed**. 10 HIGH + 23 MEDIUM + 6 LOW = **39**, not 42. The source doc holds 26 MEDIUM. The cause is traceable: the `AR-GAP` correction of the same day updated the HEADLINE totals (30 -> 42 confirmed, 6 -> 10 HIGH) and left the collective row carrying its pre-correction MEDIUM count. **This is exactly the gap a defer-grep cannot find** - the plan did not say "23 of 26", it said "23" as though that were all of them, and every subsequent reader would have reconciled 39 against 42 by trusting whichever number they read first. Corrected to `MEDIUM x26` / "remaining 32". Found by counting the SOURCE rather than by reading the plan |
| **AR6-ROWS** | **MEDIUM** | **v1.4 backlog, phase 1 - BEFORE the AR MEDIUM/LOW work starts** | **All 32 AR MEDIUM/LOW findings are homed only COLLECTIVELY, which this plan has already ruled insufficient once.** `AR-6` is a single row standing for 32 findings, each of which has a title, a unit and a refuter verdict in the source doc. `M-M12`'s own row records why that is not a home: "Cluster 2 is a UNIT-level work-order row, so M-M12 was covered only by being one of an unnamed nine... a claim of 'scheduled' that a defer-grep cannot verify because no row says it." The same shape, 32 times. The fix is not 32 hand-written rows but a DERIVED index: emit one line per source finding (id, severity, unit, one-line title, state) into the audit dir and reference it from `AR-6`, so the roster comes from the source document and cannot drift from it again - the same detect-don't-list discipline applied to `REQUIRED_HOOKS`, `_LOCAL_HOOKS_FLOOR`, the A2 integration roster and `ENTRY-GUARD`. Until that exists, "the AR MEDIUMs are scheduled" is a claim no gate can check |

### Meta-review, 2026-08-08 - probed out of THIS session's own fixes

Three probes, one per class the session's fixes belong to. Two came back clean; the third found
that I had fixed an instance and left its class.

| probe | class | verdict |
|---|---|---|
| SUP-1 | a `*`-quantified prefix class that also matches the EMPTY string | **CLEAN.** The only remaining occurrence is the predecessor regex quoted as a STRING in `mutation_check`'s B4 anchor - code as data, not live code |
| SKIP-1 | a timestamp recomputed on every write | **CLEAN.** The one other `date.today()` is the weekly MARKER write, where "now" is the correct value |
| DOCX-1 | **multiple readers for one input, first-success-wins, agreement unasserted** | **NOT CLEAN - see PDF-1** |
